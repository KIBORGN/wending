...
interface RSVPFormProps {
  builderMode?: boolean;
  hiddenFields?: Partial<
    Record<"name" | "attendance" | "children" | "drinks" | "food", boolean>
  >;
}
...
export default function RSVPForm({
  ...
  const showName = !hiddenFields.name;
  const showAttendance = !hiddenFields.attendance;
  const showChildren = !hiddenFields.children;
  const showDrinks = !hiddenFields.drinks;
  const showFood = !hiddenFields.food;
  ...
  const validate = () => {
    ...
    if (showName && !form.name.trim()) e.name = r.nameError;
    if (showAttendance && !form.attendance) e.attendance = r.attendanceError;
    if (showDrinks && form.drinks.length === 0) e.drinks = r.drinksError;
    return e;
  };
  ...
  const toggleDrink = (idx: number) => {
    ...
    setForm((f) => ({
      ...f,
      drinks: f.drinks.includes(drinkName)
        ? f.drinks.filter((x) => x !== drinkName)
        : [...f.drinks, drinkName],
    }));
  ...
  const toggleFood = (idx: number) => {
    ...
    setForm((f) => ({
      ...f,
      food: f.food.includes(foodName)
        ? f.food.filter((x) => x !== foodName)
        : [...f.food, foodName],
    }));
  };
      ...
      <div>
        <ScrollReveal stagger>
          ...
          >
                    ...
                    style={{
                      ...
                      background:
                        form.attendance === opt.val
                          ? "var(--form-field-selected-bg, #fdf4f1)"
                          : "var(--form-field-bg, #fff)",
                    }}
                  >
                    <div
                      style={{
            ...
            <button
              type={builderMode ? "button" : "submit"}
              className="btn-rose"
              disabled={loading || blocked}
              onClick={builderMode ? () => setSuccess(true) : undefined}
              style={{
                marginTop: 8,
